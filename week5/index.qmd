...
## Descriptive summary statistics tables - presentation
...
mtcars |> 
  ...
  select(car, wt, disp) |> 
  head() |> 
  knitr::kable(col.names = c("Car brand", "Weight (1000 lbs)", "Displacement (cubic inches)"), escape = FALSE) |> 
  kable_classic(full_width = FALSE)
```
...
* Besides the contents of the table, a table may be accompanied with: **_table header_**, **_caption_**, **_footnotes_** and/or **_source notes_**.
...
* You can read the documentation for each package to make the table you want (I mainly use `knitr::kable`, `kableExtra` and `DT`).
...
* When you want to **_show exact values_** or the **_accuracy of the values is important_** to convey.
